...
const PopularLabTests = () => {
  ...
  const filteredPackages =
    selectedTab && bookPopularLabTestList
      ? bookPopularLabTestList.filter((pkg) => {
          return pkg.subCategories.includes(selectedTab.toUpperCase());
        })
      : [];
  return (
    <div className="w-full md:w-4/5 mx-auto px-3">
      ...
      <div>
        {filteredPackages.map((popularLabTab) => (
          <div
            key={popularLabTab.id}
            className="relative border shadow-md rounded-2xl mt-3 p-2 md:p-4 cursor-pointer hover:transition hover:scale-105 hover:delay-200 hover:shadow-lg hover:ease-linear"
          >
